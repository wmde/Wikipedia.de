...
function getSecsPassed() {
	var startDate = replaceWikiVars( '{{{donations-date-base}}}' ).split( '-' ),
...
function getApprDonationsRaw( rand ) {
	var startDonations = parseInt( replaceWikiVars( '{{{donations-collected-base}}}' ), 10 ),
...
function getApprDonatorsRaw( rand ) {
	var startDonators = parseInt( replaceWikiVars( '{{{donators-base}}}' ), 10 ),
		secsPast = getSecsPassed();
...
function getApprDonatorsFor( secsPast, rand ) {
	var apprDonatorsMinute = parseFloat( replaceWikiVars( '{{{appr-donators-per-minute}}}' ) ),
		randFactor = 0;
...
function increaseImpCount() {
	var impCount = parseInt( $.cookie( cookieName ), 10 ) || 0;
	$.cookie( cookieName, impCount + 1, { expires: 7, path: '/' } );
	return impCount + 1;
}
